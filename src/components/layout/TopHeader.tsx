...
export default function TopHeader() {
    ...
    return (
        <header className="h-20 bg-black border-b border-white/10 flex items-center justify-between px-8 sticky top-0 z-50">
            {/* Logo Section */}
            ...
            <nav className="hidden xl:flex items-center bg-white/[0.03] border border-white/10 rounded-full px-2 py-1.5">
                {navLinks.map((link) => {
                    ...
                    return (
                        ...
                        >
                            {/* <link.icon size={14} className={isActive ? "text-black" : "text-gray-500 group-hover:text-white"} /> */}
                            {link.label}
                        </Link>
                    );
                })}
